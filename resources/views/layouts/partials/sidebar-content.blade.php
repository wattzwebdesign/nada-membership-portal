...
<div class="flex items-center px-4 py-5 shrink-0">
    <a href="{{ route('dashboard') }}">
        <img src="{{ asset('NADAWebsiteLogo.svg') }}" alt="NADA" class="h-10 brightness-0 invert" />
    </a>
</div>

{{-- Welcome section --}}
<div class="px-4 pb-4 mb-4 border-b border-white/10">
    <div class="flex items-center gap-3">
        @if(Auth::user()->profile_photo_url)
            <img src="{{ Auth::user()->profile_photo_url }}" alt="" class="w-10 h-10 rounded-full object-cover shrink-0" />
        @else
            <div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold text-white shrink-0" style="background-color: #d39c27;">
                {{ Auth::user()->initials }}
            </div>
        @endif
        <div class="min-w-0">
            <p class="text-sm font-medium text-white truncate">Welcome back, {{ Auth::user()->first_name }}!</p>
            <span class="inline-flex items-center mt-1 px-2 py-0.5 rounded-full text-xs font-medium
                ...
                {{ Auth::user()->hasRole('admin') ? 'Admin' : (Auth::user()->hasRole('registered_trainer') ? 'Trainer' : 'Member') }}
            </span>
        </div>
    </div>
</div>
...
<nav class="flex-1 overflow-y-auto px-3 space-y-6">
    {{-- MEMBER section --}}
    <div>
        <p class="px-3 mb-2 text-xs font-semibold uppercase tracking-wider text-white/40">Member</p>
        <div class="space-y-1">
            ...
            <x-sidebar-link :href="route('membership.index')" :active="request()->routeIs('membership.*')">
                <x-slot name="icon">
            ...
            <x-sidebar-link :href="route('trainings.index')" :active="request()->routeIs('trainings.*')">
                <x-slot name="icon">
            ...
            <x-sidebar-link :href="route('clinicals.index')" :active="request()->routeIs('clinicals.*')">
                <x-slot name="icon">
            ...
            <x-sidebar-link :href="route('bookmarks.index')" :active="request()->routeIs('bookmarks.*')">
                <x-slot name="icon">
            ...
            <x-sidebar-link :href="route('public.resources.index')" :active="request()->routeIs('public.resources.*')">
                <x-slot name="icon">
    ...
    @if(auth()->user()->isTrainer())
        <div>
            <p class="px-3 mb-2 text-xs font-semibold uppercase tracking-wider text-amber-400/70">Trainer</p>
            <div class="space-y-1">
...
<div class="shrink-0 border-t border-white/10 px-3 py-4 space-y-1">
    <x-sidebar-link :href="route('account.edit')" :active="request()->routeIs('account.*')">
        <x-slot name="icon">
    ...
    <x-sidebar-link :href="route('discount.request.status')" :active="request()->routeIs('discount.*')">
        <x-slot name="icon">
    ...
    <form method="POST" action="{{ route('logout') }}">
        @csrf
        <button type="submit" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10 transition-colors duration-150 w-full">
