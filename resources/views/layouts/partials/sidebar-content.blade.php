...
{{-- Welcome section --}}
<div class="px-4 pb-4 mb-4 border-b border-black/10">
    <div class="flex items-center gap-3">
        @if(Auth::user()->profile_photo_url)
            <img src="{{ Auth::user()->profile_photo_url }}" alt="" class="w-10 h-10 rounded-full object-cover shrink-0" />
        @else
            <div class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-semibold text-white shrink-0" style="background-color: #a5741e;">
                {{ Auth::user()->initials }}
            </div>
        @endif
        <div class="min-w-0">
...
<div class="flex-1 relative min-h-0" x-data="{
    ...
    <div>
        <p class="px-3 mb-2 text-xs font-semibold uppercase tracking-wider text-brand-accent">Customer</p>
        <div class="space-y-1">
    ...
    <div>
        ...
        <div class="space-y-1">
            ...
            <x-sidebar-link :href="route('membership.index')" :active="request()->routeIs('membership.*')" data-guide="nav-membership">
                <x-slot name="icon">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Zm6-10.125a1.875 1.875 0 1 1-3.75 0 1.875 1.875 0 0 1 3.75 0Zm-3.375 3.375A2.625 2.625 0 0 0 4.5 15.375v.75h7.5v-.75a2.625 2.625 0 0 0-2.625-2.625h-2.25Z" /></svg>
                </x-slot>
                Membership
            </x-sidebar-link>

            <x-sidebar-link :href="route('invoices.index')" :active="request()->routeIs('invoices.*')" data-guide="nav-invoices">
                <x-slot name="icon">
            ...
            <x-sidebar-link :href="route('trainings.index')" :active="request()->routeIs('trainings.*')" data-guide="nav-trainings">
                <x-slot name="icon">
            ...
            <x-sidebar-link :href="route('events.my-registrations')" :active="request()->routeIs('events.*')">
                <x-slot name="icon">
            ...
            <x-sidebar-link :href="route('clinical-logs.index')" :active="request()->routeIs('clinical-logs.*') || request()->routeIs('clinicals.*')" data-guide="nav-clinicals">
                <x-slot name="icon">
            ...
            <x-sidebar-link :href="route('public.resources.index')" :active="request()->routeIs('public.resources.*')" target="_blank" rel="noopener noreferrer" data-guide="nav-resources">
                <x-slot name="icon">
            ...
            <x-sidebar-link :href="route('public.glossary.index')" :active="request()->routeIs('public.glossary.*')" target="_blank" rel="noopener noreferrer" data-guide="nav-glossary">
                <x-slot name="icon">
    ...
    @if(auth()->user()->isTrainer())
        <div>
            ...
            <div class="space-y-1">
                ...
                @php
                    $pendingGroupRequests = \App\Models\GroupTrainingRequest::where('trainer_id', auth()->id())
                        ->whereNotNull('paid_at')
                        ->whereDoesntHave('training')
                        ->count();
                @endphp
                <x-sidebar-link :href="route('trainer.group-requests.index')" :active="request()->routeIs('trainer.group-requests.*')" data-guide="nav-trainer-group-requests">
                    <x-slot name="icon">
                    ...
                    @if($pendingGroupRequests > 0)
                        <span class="ml-auto inline-flex items-center justify-center px-1.5 py-0.5 rounded-full text-xs font-bold bg-red-500 text-white">{{ $pendingGroupRequests }}</span>
                    @endif
                </x-sidebar-link>
                ...
                <x-sidebar-link :href="route('trainer.clinical-logs.index')" :active="request()->routeIs('trainer.clinical-logs.*') || request()->routeIs('trainer.clinicals.*')" data-guide="nav-trainer-clinicals">
                    <x-slot name="icon">
                ...
                <x-sidebar-link :href="route('trainer.payouts.reports')" :active="request()->routeIs('trainer.payouts.reports')">
                    <x-slot name="icon">
...
<div class="shrink-0 border-t border-black/10 px-3 py-4 space-y-1">
    ...
    <div class="relative" x-data="{ open: false }" @click.outside="open = false">
        ...
        <div x-show="open"
            ...
            class="absolute bottom-full left-0 right-0 mb-1 rounded-lg shadow-lg border border-black/10 bg-white z-50 py-1"
            style="display: none;"
        >
            <a href="{{ route('account.edit') }}" data-guide="nav-account-settings" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium hover:bg-black/5 transition-colors duration-150 text-brand-text mx-1">
            ...
            @endphp
            @if($stateLawUrl)
            <a href="{{ $stateLawUrl }}" target="_blank" rel="noopener noreferrer" data-guide="nav-state-laws" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium hover:bg-black/5 transition-colors duration-150 text-brand-text mx-1">
                <span class="w-5 h-5 shrink-0">
            ...
            <a href="{{ route('public.shop.index') }}" target="_blank" rel="noopener noreferrer" data-guide="nav-shop" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium hover:bg-black/5 transition-colors duration-150 text-brand-text mx-1">
                <span class="w-5 h-5 shrink-0">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 1 0-7.5 0v4.5m11.356-1.993 1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 0 1-1.12-1.243l1.264-12A1.125 1.125 0 0 1 5.513 7.5h12.974c.576 0 1.059.435 1.119 1.007ZM8.625 10.5a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm7.5 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Z" /></svg>
                </span>
                Shop
            ...
            <a href="{{ route('public.trainers.index') }}" target="_blank" rel="noopener noreferrer" data-guide="nav-trainers-directory" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium hover:bg-black/5 transition-colors duration-150 text-brand-text mx-1">
                <span class="w-5 h-5 shrink-0">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z" /></svg>
                </span>
                Trainers Directory
        ...
        {{-- Trigger button --}}
        <button @click="open = !open" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium hover:bg-black/5 transition-colors duration-150 w-full text-brand-text">
            <span class="w-5 h-5 shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" /></svg>
            </span>
            Account Actions
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 ml-auto transition-transform duration-200" :class="open ? 'rotate-180' : ''"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 15.75 7.5-7.5 7.5 7.5" /></svg>
        </button>
    </div>

    @role('admin')
    <x-sidebar-link :href="url('/admin')" :active="request()->is('admin*')">
        <x-slot name="icon">
    ...
    <form method="POST" action="{{ route('logout') }}">
        @csrf
        <button type="submit" data-guide="nav-logout" class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium hover:bg-black/5 transition-colors duration-150 w-full text-brand-text">
            <span class="w-5 h-5 shrink-0">
