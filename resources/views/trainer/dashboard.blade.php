<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- Pending Group Training Requests --}}
            @if ($pendingGroupRequests->count() > 0)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border-l-4 border-brand-secondary">
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-3">
                                ...
                                <h3 class="text-lg font-semibold text-brand-primary">Group Training Requests</h3>
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-red-500 text-white">{{ $pendingGroupRequests->count() }}</span>
                            </div>
                            <a href="{{ route('trainer.group-requests.index') }}" class="text-sm font-medium hover:underline text-brand-secondary">View All</a>
                        </div>

                        @foreach ($pendingGroupRequests as $groupReq)
                            <div class="border-b border-gray-100 py-3 last:border-0">
                                <div class="flex items-center justify-between">
                                    <div class="flex-1 min-w-0">
                                        <a href="{{ route('trainer.group-requests.show', $groupReq) }}" class="text-sm font-medium hover:underline text-brand-primary">
                                            {{ $groupReq->training_name }}
                                        </a>
                                        <div class="flex items-center gap-2 mt-1">
                                            <p class="text-xs text-gray-500">{{ $groupReq->company_full_name }} &middot; {{ $groupReq->training_date->format('M j, Y') }}</p>
                                            <span class="text-xs text-gray-400">{{ $groupReq->members_count }} members</span>
                                        </div>
                                    </div>
                                    <a href="{{ route('trainer.trainings.create', ['from_request' => $groupReq->id]) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-md text-white bg-brand-primary hover:bg-brand-accent transition">
