    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @elseif ($clinical->status === 'approved' && !$hasCertificate)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold mb-4" style="color: #374269;">Issue Certificate</h3>
                        <p class="text-sm text-gray-600 mb-4">This clinical has been approved. You can now issue the NADA certificate for this member.</p>
                        <form method="POST" action="{{ route('trainer.clinicals.issue-certificate', $clinical) }}" onsubmit="return confirm('Are you sure you want to issue a certificate for this member? This action cannot be undone.')">
                            @csrf
                            <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white hover:opacity-90 transition" style="background-color: #d39c27;">
            ...
            @elseif ($clinical->status === 'approved' && $hasCertificate)
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div class="flex items-center gap-2">
                        ...
                        <p class="text-sm font-medium text-blue-800">Certificate Already Issued</p>
                    </div>
                    <p class="text-sm text-blue-700 mt-1 ml-7">This member already has a NADA certificate on file.</p>
                </div>
            @endif
