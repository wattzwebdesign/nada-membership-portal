        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    ...
                    {{-- Mark Complete (inline in header when threshold met) --}}
                    @if ($log->status === \App\Enums\ClinicalLogStatus::InProgress && $log->meets_threshold)
                        <div class="mt-4 p-4 bg-green-50 border border-green-200 rounded-md flex flex-col sm:flex-row items-center justify-between gap-3">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center shrink-0">
                                    <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                </div>
                                <div>
                                    <p class="text-sm font-medium text-green-800">Ready to Submit</p>
                                    <p class="text-xs text-green-700">You've met the {{ number_format($threshold, 0) }}-hour requirement.</p>
                                </div>
                            </div>
                            <form method="POST" action="{{ route('clinical-logs.complete', $log) }}" onsubmit="return confirm('Are you sure? Once submitted, you cannot add or edit entries until a trainer reviews it.')">
                                @csrf
                                <button type="submit" class="inline-flex items-center px-5 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 transition whitespace-nowrap">
