            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-lg font-semibold text-brand-primary">Product Details</h3>
                        @php
                            $editStatusColors = [
                        ...
                        @endphp
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $editStatusColor }}">
                    ...
                    <form method="POST" action="{{ route('vendor.products.update', $product) }}" enctype="multipart/form-data" x-data="{ isDigital: {{ old('is_digital', $product->is_digital) ? 'true' : 'false' }}, trackStock: {{ old('track_stock', $product->track_stock) ? 'true' : 'false' }} }">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-opacity-50 sm:text-sm">{{ old('description', $product->description) }}</textarea>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div x-show="!isDigital" class="border border-gray-200 rounded-lg p-4 space-y-4">
                                ...
                                <div x-show="trackStock" x-cloak>
                                    ...
                                    <input type="number" name="stock_quantity" id="stock_quantity" value="{{ old('stock_quantity', $product->stock_quantity) }}" min="0" class="mt-1 block w-full sm:w-1/3 rounded-md border-gray-300 shadow-sm focus:ring-opacity-50 sm:text-sm">
